# CLAUDE.md — ai-sites-course

## What is this project
Ukrainian no-code website builder course platform. Teaches creating websites with AI-powered tools (Durable, Mixo, Wix AI) — no coding required.
4 modules, 14 lessons. Ukrainian language UI.
...
1. **ai-sites-course** (цей проєкт) — сайт без коду: AI-білдери, візитки, лендінги
2. **figma-course** — Figma для новачків: компоненти, дизайн-системи, прототипи
...
npm run build  # production build — must produce 22 static pages
...
lib/
  courseData.ts           # ALL course content (1402 lines, ~79KB) — modules, lessons, quizzes
  progressContext.tsx     # Progress state via localStorage ("ai_sites_progress")
...
| `--accent` | Green — CTAs, active states (`#22c55e` dark / `#16a34a` light) |
| `--accent-hover` | Lighter/darker green on hover |
...
### 4. Static generation
All 19 lesson pages pre-built via `generateStaticParams()` in `page.tsx`.
Always run `npm run build` after changing `courseData.ts` or route structure.
Expected output: **22 static pages** (19 lessons + homepage + /course + /not-found).

## Course data (lib/courseData.ts)

File is 1402 lines / ~79KB — source of truth for all course content. Do not duplicate content elsewhere.
...
- Lesson ID format: `m{n}-l{n}` (e.g. `m1-l1`, `m3-l9`) — lesson numbers are global (not reset per module)
- Quiz ID format: `m{n}-quiz` (e.g. `m1-quiz`)
- Artifact ID: `m4-artifact`

**Module titles:**
- module-1: "Розминка: Знімаємо страх перед технологіями" (3 lessons + quiz)
- module-2: "Швидка перемога: Сайт-візитка за 15 хвилин" (3 lessons + quiz)
- module-3: "Рівень «Топ-Лендінг»: Сайт, що продає" (4 lessons + quiz)
- module-4: "Місток до Інтернет-магазину" (4 lessons + quiz + artifact)
...
- `localStorage` key: `"ai_sites_progress"` (JSON array of completed lesson IDs)
- `TOTAL_LESSONS = 19` — update this constant if lesson count changes
...
- **After every change:** run `npm run build` to verify all 22 pages build successfully.
